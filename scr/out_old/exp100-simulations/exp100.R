setwd("~/Sync/DOCUMENTS/onderzoek/LiXue/bayesdec_julia/exp100-simulations")
...
post_tau <- read_csv("methodDtest/post_tau.csv",col_names = FALSE)
post_tau %>% mutate(iterate=1:nrow(post_tau)) %>% ggplot() + 
  geom_line(aes(x=iterate,y=X1))
